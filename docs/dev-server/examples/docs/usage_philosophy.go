...
func UsagePhilosophy() dashboard.Dashboard {
	return dashboard.New().
		WithLayout(layout.DocsPage).
		Widget(
			widget.NewMarkdown().
				Title("Usage Philosophy").
				Content(`
# Usage Philosophy
...
Thus, if you have a wide format (with multiple observations per row), you can use ClickHouse [Array Join](https://clickhouse.com/docs/sql-reference/functions/array-join) to convert it to a [narrow format](https://en.wikipedia.org/wiki/Wide_and_narrow_data) - this is also called *unpivot* of data.

## Next Steps

- [Installation](/docs/installation) - Set up your Dashica project
- [Quick Start](/docs/quickstart) - Create your first dashboard
- [Queries](/docs/queries) - Learn about SQL query building
`),
		)
}
