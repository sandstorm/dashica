...
// Test constants
const (
	timeHeatmapOrdinalTestTableName   = "events"
	timeHeatmapOrdinalTestWhereClause = "timestamp > now() - INTERVAL 1 DAY"
)

// Test helpers
func newTimeHeatmapOrdinalTestBaseQuery() *sql.SqlQuery {
	return sql.New(
		sql.From(timeHeatmapOrdinalTestTableName),
		sql.Where(timeHeatmapOrdinalTestWhereClause),
	)
}

func TestTimeHeatmapOrdinal_BuildChartProps(t *testing.T) {
	tests := []struct {
		name     string
		setup    func(*TimeHeatmapOrdinal) *TimeHeatmapOrdinal
		expected map[string]interface{}
	}{
		{
			name: "Basic configuration with required fields only",
			setup: func(h *TimeHeatmapOrdinal) *TimeHeatmapOrdinal {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			widget := NewTimeHeatmapOrdinal(baseQuery)
...
func TestTimeHeatmapOrdinal_SQLGeneration(t *testing.T) {
	tests := []struct {
		name        string
		setup       func(*TimeHeatmapOrdinal) *TimeHeatmapOrdinal
		expectedSQL string
...
ORDER BY
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			widget := NewTimeHeatmapOrdinal(newTimeHeatmapOrdinalTestBaseQuery())
			widget = tt.setup(widget)
...
func TestTimeHeatmapOrdinal_Immutability(t *testing.T) {
	// Test that the fluent API returns new instances and doesn't mutate the original
	original := NewTimeHeatmapOrdinal(sql.New(sql.From("test")))
...
func TestTimeHeatmapOrdinal_BuildComponents(t *testing.T) {
	baseQuery := sql.New(sql.From(timeHeatmapOrdinalTestTableName))
	widget := NewTimeHeatmapOrdinal(baseQuery).
...
func TestTimeHeatmapOrdinal_BuildComponents_AutoGeneratesId(t *testing.T) {
	baseQuery := sql.New(sql.From(timeHeatmapOrdinalTestTableName))
	widget := NewTimeHeatmapOrdinal(baseQuery).
		X(sql.Timestamp15Min()).
		Y(sql.Field("status")).
